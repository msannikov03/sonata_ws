...
RESULTS_PATH = RESULTS_DIR / "teacher_without_scaffold_fair_500.json"
NUM_FRAMES = 500
DEVICE = "cuda"
MAX_PTS = 20000
SEED = 42
...
def load_frames(n=NUM_FRAMES):
    all_files = sorted(PREVOX_DIR.glob("*.npz")); stride = max(1, len(all_files) // n); files = all_files[::stride][:n]
    frames = []
    for f in files:
        d = np.load(f)
        frames.append({
